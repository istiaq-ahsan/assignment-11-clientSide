...
const SixMarathon = () => {
  ...
  const fetchSixMarathon = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_API_URL}/six-marathons`,
      sixMarathon
    );
    setSixMarathon(data);
  };
    ...
    <div className="py-20">
      <div className="text-center">
        ...
        <p className="text-base my-10 w-4/5 mx-auto">
          Join us for thrilling marathons and races across the country! Whether
          you're a seasoned runner or a first-time participant, our events cater
          to all levels.
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5 w-11/12 mx-auto">
        {sixMarathon.map((marathon, index) => (
          <SixMarathonCard key={index} marathon={marathon}></SixMarathonCard>
        ))}
      </div>
    </div>
  );
